...
p_base.sigma       = 0.5;    % Baseline market share (m_0)
...
p_dec.sigma       = 0.60;    % UPDATED: Swapped to 0.30
p_dec.T           = 10;      
p_dec.tau_s       = 1.20;    
...
p_inc.sigma       = 0.40;    % UPDATED: Swapped to 0.20
p_inc.T           = 16;      
p_inc.tau_s       = 0.80;    
...
% --- COMPUTE BASELINE ---
baseline_val = calcVoucherValue(p_base, salesVec);
...
for i = 1:num_params
    ...
    res_dec(i) = calcVoucherValue(p_test_dec, salesVec);
    
    % Test "Increases Value" bound
    p_test_inc = p_base; 
    p_test_inc.(param) = p_inc.(param); 
    res_inc(i) = calcVoucherValue(p_test_inc, salesVec);
end
...
spread = abs(res_inc - baseline_val)/baseline_val;
[~, sort_idx] = sort(spread, 'ascend'); 
...
function VoucherValue_comp = calcVoucherValue(p, salesVec)
    ...
    for t = 1:T_val
        npv_with_both = npv_with_both + ...
            p.rho * factor * adjustedSales(t)*(1+4*Delta_tau_e*p.Delta_sigma/p.sigma) / (1 + p.i_rate)^(t + p.tau_s + p.tau_ea);
    end
    
    % Add the final year residual (using the very last element of our adjusted vector)
    npv_with_both = npv_with_both + ...
        p.rho * Delta_tau_e * factor * adjustedSales(end)*(1+4*Delta_tau_e*p.Delta_sigma/p.sigma) / (1 + p.i_rate)^(T_val + p.tau_s + p.tau_ea);
